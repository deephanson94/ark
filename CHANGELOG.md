...
- **M0 — atlas format, indexer, verb contracts.** Wrote `docs/atlas-format.md` (schema v1) and the
  ...
  and `Grade` contracts plus F1 set scoring in `src/verbs/`; wired `test:unit` (119), `test:atlas`
  (57) and `test:determinism` (two independent processes, byte-diffed — verified failing first
  against an injected timestamp and an unseeded layout). Indexes this repo in 94 ms to a 25.6 KiB
  atlas, 48 nodes, 77 edges, **zero unresolved imports**. Seven ADRs in `docs/decisions/` cover the
