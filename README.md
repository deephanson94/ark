...
|---|---|---|---|---|
| **Blast Radius** | *"Which of these files depend on this one?"* | Transitive dependents, unbounded | 3 | ✅ — but **fully supplied on 8 of 19 measured repos**, token on 7, absent on 4 ([ADR-0042](./docs/decisions/0042-blast-radius-is-taint-limited-on-half-the-real-repositories.md)). **The only verb whose failed boards are re-earnable** ([ADR-0053](./docs/decisions/0053-a-failed-board-is-re-earnable-on-a-question-it-did-not-answer.md)): 75/50/89/88% of its boards on ark, hono, kysely and graphql-js carry a second, disjoint window. |
...
- **Only Blast Radius's failed boards can be re-earned.** A second, disjoint window is the only
  honest re-earn ([ADR-0053](./docs/decisions/0053-a-failed-board-is-re-earnable-on-a-question-it-did-not-answer.md)),
  and it is wired for one verb of four. The rest is supply, not effort: the arithmetic ceiling — a
  population at least twice the answer key — is **Companion 41–64%, Archaeology 24–56% and Placement
  4–20%** across ark, hono, kysely and graphql-js (`npm run probe:retry`, at `e0bb4bf`). Placement is
  the floor because its subject is a commit and a commit's file list is usually about the size of the
  key sampled out of it, so most of its boards can never be re-earned by any design. Those three
  verbs behave exactly as they did before: fail one, and a later pass is `shown`.
