...
|---|---|---|---|---|
| **Blast Radius** | *"Which of these files depend on this one?"* | Transitive dependents, unbounded | 3 | ✅ — but **fully supplied on 8 of 19 measured repos**, token on 7, absent on 4 ([ADR-0042](./docs/decisions/0042-blast-radius-is-taint-limited-on-half-the-real-repositories.md)). **Failed boards are re-earnable** ([ADR-0053](./docs/decisions/0053-a-failed-board-is-re-earnable-on-a-question-it-did-not-answer.md)): 75/50/89/88% of its boards on ark, hono, kysely and graphql-js carry a second, disjoint window — the best supply of any verb. |
| **Companion** | *"Which files change with this one?"* | Co-change matrix | 3 | ✅ — **failed boards are re-earnable** on 33/31/59/57% of its boards ([ADR-0053](./docs/decisions/0053-a-failed-board-is-re-earnable-on-a-question-it-did-not-answer.md)); the pair-claiming rule takes most of the gap to its 45–64% ceiling. |
| **Placement** | *"This commit landed — which files did it change?"* | The commit's own file list | 6 | ✅ — its rows now carry **each candidate's own churn and last-seen** ([ADR-0052](./docs/decisions/0052-a-row-may-carry-what-its-own-gate-already-prices.md)), because `channel: 'nothing'` left the map silent while its board is open and three round-7 testers read it as word-matching. Free against its gate, which already priced both columns — and the conjunction, which it did not, is priced now. |
...
| Ctrl+F gate (pillar 3, made computable) | ✅ | One heuristic set per verb, scored against band A; admission rule stated in ADR-0021 *(this row said "nine heuristics" while there were ten, which is why it now names the rule instead of the count)*. Its one *accepted* exposure is now closed rather than held under the bar — the structure-blind subtree hint is withheld, since a margin of 0.011 on a self-indexing repo lasted three milestones.<br><br>**`churn` and `recency` were both scored, and their conjunction was not** — a date filter returning more rows than the key needs can be truncated by churn, raising precision without costing recall, and neither alone bounds that. It was a priced-but-unavailable guess for as long as both numbers lived in an inspector an open board makes unreachable; printing them on Placement's candidate rows made it available and `datedChurn` prices it ([ADR-0052](./docs/decisions/0052-a-row-may-carry-what-its-own-gate-already-prices.md)). **ark, hono, kysely and graphql-js all read 0 boards beaten; django read 2 and svelte 2, at a flat 1.000** — four repos said the channel was free and the fifth and sixth said it hands out an exact key. Afterwards 0 on all six, best 0.750 against the 0.78 bar; the cost is **one board each** on django (273 → 272) and svelte (235 → 234) and none on the four cap-limited repos. It has two readings and each is the only one that fires somewhere, so both are scored and the better taken. |
...
| Proof is what you were not told | ✅ | **Every answer gets its full reveal**, and the defence moved to the ledger: a pass is recorded as `proved` or `shown` and its field note says which ([ADR-0047](./docs/decisions/0047-proof-is-what-the-first-answer-earned.md), **reverses owner-decided ADR-0035; ratified 2026-08-14**; its *first submission or nothing* rule is superseded below). The gate it replaces withheld the reveal below 0.5 precision and could not work: a single pick scores above zero exactly when it is in the key, so the *score* is a membership oracle — **952 of 952 boards on four repos** — and its own showcase case *was* the exploit (pick one file right → precision 1.0 → the whole annotated key and the drawn cone, without passing → reopen and type it back). `understood` reads the proved register only; the deck, the cone and the note read either. A `graded` key **decays with the pass it certifies**, or a re-rolled board would come back permanently unprovable.<br><br>**A failed board is re-earnable now, and only on a question it did not answer for you** ([ADR-0053](./docs/decisions/0053-a-failed-board-is-re-earnable-on-a-question-it-did-not-answer.md); owner's decision 2026-08-25, amending ADR-0047 decision 2). That decision's *reason* was always about members — a later pass certifies nothing **because the board already named them** — so the rule is now `proved` unless a claimed member was already named, which reproduces every case the attempt-number rule decided (ADR-0047's two farm suites pass **unchanged**) and adds the one it could not express. The re-earn itself is a **second, disjoint window** on the board, because `Grade.missed` is `truth \ picked` and §8.1 requires an honest grade, so a failing submission prints the whole key by name and no policy over the *panel* can change that. **Two verbs of four**: Blast Radius ships one on 75/50/89/88% of its boards and Companion on 33/31/59/57% (ark / hono / kysely / graphql-js), against arithmetic ceilings of 78/56/95/88% and 45/41/64/62%. Archaeology's ceiling is 24–56% and **Placement's is 4–20%** — a commit's file list is usually about the size of the key sampled from it, so most of its boards can never be re-earned by any design; both are unwired. **The deck is byte-identical with and without retries** on all four repos, which is the acceptance test and not a nicety: claiming a Companion retry's pairs *inside* the build loop re-rolled **20 of ark's 40 boards onto different subjects** and re-keyed 12 more, so the second window is computed after the cap. Atlas +3.6–5.8% for one verb, 449.7 → 492.1 KiB for both; `ATLAS_VERSION` 11 → 12, saves untouched. |
...
- **Two verbs of four can re-earn a failed board.** A second, disjoint window is the only honest
  re-earn ([ADR-0053](./docs/decisions/0053-a-failed-board-is-re-earnable-on-a-question-it-did-not-answer.md));
  Blast Radius and Companion ship one, Archaeology and Placement do not. The rest is supply, not
  effort: the arithmetic ceiling — a population at least twice the answer key — is **Archaeology
  24–56% and Placement 4–20%** across ark, hono, kysely and graphql-js (`npm run probe:retry`).
  Placement is the floor because its subject is a commit and a commit's file list is usually about the
  size of the key sampled out of it, so most of its boards can never be re-earned by any design.
  Those two verbs behave exactly as they did before: fail one, and a later pass is `shown`.
- **Two columns and no reasoning still reach a bare pass on a quarter to a half of Placement's boards.**
  ...
  *"the files in this folder are coupled" is cheap but true*. It is a gap and not a defect, the honest
  comparison is against the **zero** the same player scored before, and lowering it means moving a bar
  that governs every verb — an owner's decision, not a session's.
- **Archaeology's rows still carry no structural fact at all, and the obvious one is not free.** Its
  candidates are commits, printing a date and a message, and its `channel` is `'nothing'` — so the
...
- **The first ten minutes score 7.4 visual and 6.8 "would you keep playing", against a goal of 8.**
  ...
  **Round 7 was flat against round 6**, which is the useful part of it: the three fixes round 6
  ...
  found instead is above — a board answerable by sorting the filenames — plus two items, one of them
  now **half closed**: three testers reported **Archaeology and Placement as word-matching rather
  than structural** (one scored 0%, *"I had no basis for that at all"*), and a **failed board is
  permanently dead** three lines under copy promising *"answer as often as you like, nothing is
  lost"*. Placement's half is answered — its rows carry each candidate's churn and last-seen
  ([ADR-0052](./docs/decisions/0052-a-row-may-carry-what-its-own-gate-already-prices.md)), a channel
  its own gate had priced and the player could not read. **Archaeology's is not**, and it has its own
  gap entry above. The structural cause was in the code rather than in the wording: three verbs of
  four declare `channel: 'nothing'` or have it gated off, so the map is scenery while their board is
  open — for Companion that is ADR-0016's argued refusal, and for Placement nobody had ever decided
  it.
