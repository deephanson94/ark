...
**A wrong answer you did not pick is never explained.** Round 5's *"make the map itself the
scoreboard"* is **done**: the region wash shipped at `66f13d7`, the proved chain at `bc3f039` and its
hop count here — so what is left of that round is the reveal. Its rows are `picked ∪ truth`, so a
perfect score teaches nothing about the other eighteen candidates · **a rig that frames what is ahead**, which is the walk's actual defect now that the collision
