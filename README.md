...
**Label the proved chain with its hop count.** Round 5's four independent requests for *"make the map
itself the scoreboard"* are now three-quarters done: the region wash shipped at `66f13d7` and **the
proved chain ships** at `bc3f039` — every hop of the route from a file you proved to the subject you
proved it about, in violet, behind ADR-0049 §7.3's gate. What is left of that quote is its second
half, *"labelled with its hop count"*, which carries no new disclosure (the cone is already unlocked
for a subject whose board is answered) and is scope rather than a refusal — ADR-0049 §7.6 says so
rather than leaving a reader to infer it from a feature that half matches a quotation · then **a
wrong answer you did not pick is never explained** (the reveal's rows are `picked ∪ truth`, so a perfect score teaches nothing
