...
- **Liveness**: nothing is reported unless import sites > 0 **and** the AST count agrees with an
  independent crude line-grep. Measured: 677 vs 695, 12,000 vs 12,099, 190 vs 200, 6,013 vs 6,151 —
  all within 3%.
...
The residue splits into two kinds, and **neither is fixable by a better parser**:

| repo | unresolved | internal-but-unrooted | external-but-undeclared |
|---|---|---|---|
| flask | 42 | 17 | 25 |
| django | 170 | 45 | 125 |
| hugo | 11 | 0 | 11 |
...
| such distractor slots | 664 |
| — of which a *different* package (`foo_test`), correctly not a dependency | 68 |
| — same package | 596 |
| — **same package AND the sibling references a package-level identifier the subject declares** | **80** |
| **boards carrying at least one verified wrong answer** | **49 of 244 (20.1%)** |

The first count of that last row was **153 slots**, from a test that counted *method* names.
Method names live in their receiver type's namespace, not the package's, so `Close`/`Create`/`Write`
matched across unrelated types — interface implementations, not references. Restricting to
package-level declarations, where Go's namespace really is unique, cut it to 80. **Test the claim,
not the wording**: the looser test would have overstated the leak by 91%.

One in five Go boards marks a player wrong for an answer that is right. That is the outcome
guardrail 4 exists to prevent, and no existing check can see it.
...
|---|---|---|---|---|
| file granularity, fan-out (what a naive port gives) | 1,955 | 25,500 | 13.04 | present on 20.1% of boards |
...
is the only one of the three that makes the map *more* legible rather than less. `AtlasNode.kind` is
already `'file' | 'dir' | 'symbol'` in the schema, so the format anticipated this.
...
1. **Go ships at M5, at *package* granularity — the node is the directory, not the file.** File
   granularity is refused: it produces a verified wrong answer key on 20.1% of boards (§6.1), a 13.04
...
6. **Re-measure §4 and §6 per language, never per family.** cobra and hugo are the same language and
   disagree on every headline number; flask and django disagree on taint by 26 points. Two repos per
...
**Ship Go at file granularity and gate the `treeSibling` strategy on same-directory pairs.** Closes
the measured leak and leaves the fan-out and the budget breach, and it is a per-row guard of exactly
the kind ADR-0020 forbids — the *absence* of a same-directory distractor would itself say "this
subject's package has more than one file".

**Add intra-package clique edges.** Sound, and it takes hugo to 42,780 edges and 21.9 edges/node
...
- **Python**: a measured way to obtain import roots and distribution-to-module mappings **without
  running anything**. `importlib.metadata` over an installed environment would do it and violates
  pillar 6. If that changed, re-run §4 first — the amplification, not the resolution rate.
