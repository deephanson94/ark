 ...
 * never looked".
 *
 * Measured at `b1ba3c6` over nine repositories, 285 to 12,626 towers:
 *
 *   repo         towers  above eye  street  canyon   fan med/p95  fan > street
 *   ark             285      22.8%    11.9   0.4:1     0.0 / 3.8         2.2%
 *   hono            425      15.8%    13.0   0.8:1     0.0 / 3.7         1.0%
 *   prometheus      501      30.9%    12.4   1.4:1     0.0 / 4.6         0.5%
 *   graphql-js      549      32.8%    11.8   0.4:1     1.1 / 11.3        3.9%
 *   kysely          600      49.2%    12.3   2.4:1     3.3 / 12.1        4.5%
 *   hugo          1,242      14.1%     9.8   0.5:1     1.4 / 7.1         3.2%
 *   django        3,035      13.4%     8.9   0.6:1     0.4 / 9.3         5.3%
 *   typeorm       3,704      13.0%     7.7   1.4:1     0.3 / 11.4        9.0%
 *   webpack      12,626       5.9%     7.4   0.7:1     0.5 / 6.8         4.6%
 *
 * `ROAD_WIDTH` holds: a fan is 0.0–3.3 units at the median against streets
 * 7.7–13.0 wide, and 0.5%–9.0% of hub nodes have one wider than the nearest
 * street. The counterfactual is in the table — at the pre-tuning 2.2 every
 * figure is 2–6× larger — so the constant does something and what it does is
 * enough.
 *
...
for (const repo of process.argv.slice(3)) {
  ...
  const swallowed = plate.filter((radius) => radius > medianGap).length;
  console.log(
